...
/*
Changes:
2009/11/09 + added support for modifying EXE-based PBDs
           + added CreateExeLibrary for appending empty PBD to EXE 
...
PBLMI_Result 
PBLMI_PBL::SeekEntry( const wchar_t *szEntryName, PBL_ENTRYINFO * pEntry, BOOL bCreate )
{
    PBLMI_Result ret;
...
PBLMI_PBL::PBLMI_PBL(PBLMI *aPbLMI, const char *szLibName, BOOL bReadWrite)
{
    ...
    trace_log << "afopen" << endl;
	m_Handle = fopen(szLibName, bReadWrite ? "rb+" : "rb");
    init();
}

PBLMI_PBL::PBLMI_PBL(PBLMI *aPbLMI, const wchar_t *szLibName, BOOL bReadWrite)
{
	m_bIsPBL = FALSE;
	m_Handle = NULL;
	m_bUTF = FALSE;
	m_iPbLMI = aPbLMI;
	m_WriteMode = bReadWrite;
    trace_log << "wfopen" << endl;
    m_Handle = _wfopen(szLibName, bReadWrite ? L"rb+" : L"rb");
    init();
}
...
void PBLMI_PBL::init()
{
	...
	}else
	{
        ...
        {
            trace_log << "no TRL" << endl;
			return;
        }
	} 
	fseek(m_Handle, m_StartOffset + 0x04, SEEK_SET);
    trace_log << "fread 5" << endl;
    if (!fread(buf, 4, 1, m_Handle))
		return;
...
PBLMI::CreateLibrary(const char *szLib, int aPbVer)
{
	...
	PBLMI_Result ret = AppendEmptyLibrary(aPbVer, f, 0);
	fclose(f);
	return ret;
}

PBLMI_Result PBLMI::CreateExeLibrary( const char *szExe, int aPbVer /*= 6*/ )
{
	FILE * f = fopen(szExe, "r+b");
	if (!f)
		return PBLMI_ACCESS_DENIED;
	fseek(f, 0, SEEK_END);
	DWORD offset = ftell(f);
	PBLMI_Result ret = AppendEmptyLibrary(aPbVer, f, offset);
	if (ret == PBLMI_OK) {
		ret = AppendTrailerRecord(f, offset);
	}
	fclose(f);
	return ret;
}
...
IPBLMI_PBL* 
PBLMI::OpenLibrary(const wchar_t *szLib, BOOL bReadWrite)
{
    PBLMI_PBL *iPbl = new PBLMI_PBL(this, szLib, bReadWrite);
    if (!iPbl->m_bIsPBL) {
        delete iPbl;
...
PBLMI_Result PBLMI::AppendEmptyLibrary( int aPbVer, FILE * f, DWORD startOffset)
{
	int retn;
	PBL_HDR_U u;
	...
	long offset;
	offset = startOffset + (m_bUTF ? 0x400 : 0x200);
	PBL_FRE fre;
...
PBLMI_Result PBLMI::AppendTrailerRecord( FILE * f, DWORD startOffset)
{
	PBL_TRL trl;
	char buf[0x200 - sizeof(trl)];
	strncpy(trl.sign, "TRL*", 4);
	memset(buf, 0, sizeof(buf));
	trl.hdr_offset = startOffset;
	fseek(f, 0, SEEK_END);
	fwrite(&trl, sizeof(trl), 1, f);
...
PBLMI_Result PBLMI_PBL::ReadFreeListData()
{
	...
	// count FRE blocks
	int fre_count = 0;
	for (fre.next = m_StartOffset + m_FreeListFirstOffset; fre.next; ) {
	...
	*/
	...
	for (int li_Block = 0; li_Block < ai_Count; li_Block ++) {
		...
		m_FreeList[li_CurBlocks + li_Block].offset = 
			m_StartOffset + (li_CurBlocks + li_Block) * BlockSize * FreeBlockBitSize;
...
PBLMI_Result PBLMI_PBL::AllocateBlocks(int ai_Count, BlockList &aList, BOOL ab_Continuous)
{
	...
	while (li_AllocatedCount < ai_Count) {
		...
		aList[li_OldSize + li_AllocatedCount] = m_StartOffset + DWORD(BlockSize * li_BlockIndex);
		li_AllocatedCount ++;
...
PBLMI_Result PBLMI_PBL::CreateNode(DWORD adw_ParentNode, DWORD &adw_NewNode)
{
	...
	if (ret != PBLMI_OK)
		return ret;
	ret = WriteData("\x0", 1, adw_NewNode + NodeSize - 1);
	if (ret != PBLMI_OK)
		return ret;
	return PBLMI_OK;
}
...
DWORD PBLMI_PBL::readExePblOffset()
{
	char buf[4];
	DWORD offset;
	fseek(m_Handle, -0x200, SEEK_END);
	if (fread(buf, 4, 1, m_Handle) != 1)
		return 0;
	if (strncmp(buf, "TRL*", 4) != 0)
		return 0;
	if (fread(&offset, sizeof(offset), 1, m_Handle) != 1)
		return 0;
	fseek(m_Handle, offset, SEEK_SET);
	if (fread(buf, 4, 1, m_Handle) != 1)
		return 0;
...
void PBLMI_PBL::writeTrailerRecord()
{
	// skip if read only
	if(!m_WriteMode)
		return;
	...
	if (m_StartOffset == readExePblOffset())
		return;
	PBL_TRL trl;
	char buf[0x200 - sizeof(trl)];
	strncpy(trl.sign, "TRL*", 4);
	memset(buf, 0, sizeof(buf));
	trl.hdr_offset = m_StartOffset;
	fseek(m_Handle, 0, SEEK_END);
	fwrite(&trl, sizeof(trl), 1, m_Handle);
	fwrite(buf, sizeof(buf), 1, m_Handle);
}

void PBLMI_PBL::Flush()
{
	if (!m_Handle)
		return;
	// write durty buffers
...
DWORD PBLMI_PBL::getStartOffset()
{
