...
class PBLMI_PBL: public IPBLMI_PBL
{
   ...
   public:
    ...
    PBLMI_Result 
    SeekEntry(const wchar_t *szEntryName, PBL_ENTRYINFO * pEntry, BOOL bCreate);
   ...
   PBLMI_Result WriteComments(void * pBuf);
    ...
    PBLMI_PBL(PBLMI *aPbLMI, const wchar_t *szLibName, BOOL bReadWrite);
...
class PBLMI : public IPBLMI
{
   ...
   public:
	...
	// create and append empty library to EXE
	PBLMI_Result 
	CreateExeLibrary(const char *szExe, int aPbVer = 6);

	// open library and return IPBLMI_PBL interface to misc. LMI functions
	IPBLMI_PBL* 
	OpenLibrary(const char *szLib, BOOL bReadWrite);
    IPBLMI_PBL* 
        OpenLibrary(const wchar_t *szLib, BOOL bReadWrite);
	...
	static PBLMI_Result AppendEmptyLibrary( int aPbVer, FILE * f, DWORD startOffset);
	static PBLMI_Result AppendTrailerRecord( FILE * f, DWORD startOffset );
};
